...
void usualWork() {
    ...
    std::vector<std::vector<int>> n =
                                   createMatrix(str1.size() + 1, str2.size() + 1);
...
void killMatrix(std::vector<std::vector<int>> matrix) {
    for (std::vector<int> row : matrix) row.clear();
    matrix.clear();
}
...
int compare(int a, std::vector<std::vector<int>> &m, size_type i, size_type j,
            QString l, QString u) {
    int c = a;
    if ((i > 1 && j > 1) && (l[i] == u[j - 1]) && (l[i - 1] == u[j])) {
        c = m[i - 2][j - 2] + 1;
    }
    if (c < a) return c;
    return a;
...
int matrixwork4(std::vector<std::vector<int>> &num, QString left, QString u) {
    ...
    for (size_type i = 1; i < usize; i++) {
        num[1][i] = minim(num[0][i - 1] + (!(left[0] == u[i - 1])),
                          num[1][i - 1] + 1, num[0][i] + 1);
    }
    for (size_type i = 2; i < lsize; i++) {
        num[i][1] = minim(num[i - 1][0] + (!(left[i - 1] == u[0])), num[i][0] + 1,
                          num[i - 1][1] + 1);
        for (size_type j = 2; j < usize; j++) {
            num[i][j] =
                compare(minim(num[i - 1][j - 1] + (!(left[i - 1] == u[j - 1])),
                              num[i][j - 1] + 1, num[i - 1][j] + 1),
                        num, i, j, left, u);
        }
    }
...
unsigned __int64 timeMatrix4Work(std::vector<std::vector<int>> num, QString l,
                                 QString u) {
...
unsigned __int64 timeMatrixWork(std::vector<std::vector<int>> num, QString l,
                                QString u) {
    unsigned __int64 startTime = 0, endTime = 0, averageTime = 0;
...
void timeMatrix(QString l, QString u) {
    ...
    // std::cout << "\n" << "время создания матрицы для строк размером " << lsize
    // << " на " << usize << " составляет " << timeCreate << " тиков.";
    // std::cout << "\n" << "время нахождения пути с помощью матрицы: " <<
    // timeCreate + timeWork << " тиков. Расстояние: " << answer << "\n";
    std::cout << "\n"
              << "the time distance with matrix " << timeCreate + timeWork
              << " ticks. Distance: " << answer << "\n";
...
int recurs_tree(QString l, QString u, int i, int j, int oldi, int oldj,
    ...
    if (j < 1) returning = i;
    if ((i > 0) && (j > 0))
        returning = minim(recurs_tree(l, u, i - 1, j - 1, i, j, counter + 1) +
                          !(l[i - 1] == u[j - 1]),
                          recurs_tree(l, u, i, j - 1, i, j, counter + 1) + 1,
                          recurs_tree(l, u, i - 1, j, i, j, counter + 1) + 1);
    std::cout << "\nmy number is " << returning << " and my step is " << oldi
              << " " << oldj;
...
int recurs(QString l, QString u, int i, int j) {
    ...
    return minim(recurs(l, u, i - 1, j - 1) + !(l[i - 1] == u[j - 1]),
                 recurs(l, u, i, j - 1) + 1, recurs(l, u, i - 1, j) + 1);
}
...
void timeRec(QString l, QString u) {
    ...
    std::cout << "\n"
              << "the time distance with recursion " << timeWork
              << " ticks. Distance: " << answer;
}
...
void test1() {
    ...
    timeSend("рентгеноэлектрокардиографического",
...
bool isMatch(std::string left, std::string up, size_type index1,
             size_type index2) {
